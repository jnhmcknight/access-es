...
def _do_logged_in_request(path, **kwargs):
    """ Makes the actual request to hosted ES """

    if not _check_login():
        return redirect(url_for('login_form'))

    allow_redirect = kwargs.get('allow_redirect', True)
...
@app.route('/_plugin/kibana/goto/<shortened_token>', methods=['GET'])
def redirect_shortened(shortened_token, **kwargs):
    """ Correctly redirects to a short URL generated by Kibana """

    path = '/_plugin/kibana/goto/{}'.format(shortened_token)

    # When performing the request for the shortened URL, we need to resign a new
    # request to the correct URL or AWS fails due to an invalid signature
    long_url_resp = _do_logged_in_request(path, allow_redirect=False)
    if not long_url_resp.headers.get('Location'):
        return long_url_resp

    return _do_logged_in_request(long_url_resp.headers['Location'])
...
def proxy_request(path, **kwargs):
    ...
    return _do_logged_in_request(path)
